...
if [ "$SUITE" = "phpunit" ];
then
    initcmd="bin/moodle-docker-compose exec -T webserver php admin/tool/phpunit/cli/init.php --prefer-source"
elif [ "$SUITE" = "behat" ];
then
    initcmd="bin/moodle-docker-compose exec -T webserver php admin/tool/behat/cli/init.php --prefer-source"
